...
class Resizer:
    def __init__(self, *, size, root, mode):
        self.size = size
        self.root = root
        self.mode = mode
    ...
    def __call__(self, index_filename):
        ...
        filename = filename.replace('img', f'img_{self.mode}')
        result_img = self.prepare(filename)
        return index, result_img, filename


def prepare_data(root, dataset_mode, out, n_worker, size, chunksize):
    ...
    lmdb_path = os.path.join(out, f'{dataset_mode}_lmdb')
    with lmdb.open(lmdb_path, map_size=1024 ** 4, readahead=False) as env:
        with env.begin(write=True) as txn:
            ...
            resizer = Resizer(size=size, root=root, mode= dataset_mode)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset_mode', type=str, default='lowers', help='a path to output directory')
    parser.add_argument('--out', type=str, help='a path to output directory')
    parser.add_argument('--size', type=int, nargs='+', default=((256, 176)))
